...
class AssignmentController extends Controller
{
  ...
  $extension = $file->getClientOriginalExtension();
    ...
    public function  insertValuesInAssignment($login_user_id,$due_date,$subject,$no_of_pages,$ref_style,$expert,$description,$uploaded_file1='',$uploaded_file2='',$uploaded_file3='',$extra_param='')
    {
        $currency ="USD";
        $response = new SendResponseController();
        $total=$no_of_pages*Config::get('page_cost_per_page_multiplier');
...
//              }
    ...
    public function showAssignmentDetails($id)
    {
       ...
       $uploaded_plagi_file_array = null;
             ...
             if(!empty($details) && $details!='')
             {
               ...
               if($uploadedFiles)
               {
                   foreach($uploadedFiles as $file)
                   {
                       ...
                       $uploaded_file['getfile']=$file->original_filename;
               ...
               $uploadedPlagiFiles=DB::table('plagi_files')->where('assignment_id',$details->assignment_id)->get();  

               if($uploadedPlagiFiles)
               {
                   foreach($uploadedPlagiFiles as $plagiFile)
                   {
                       $uploaded_plagi_file['filename']=$plagiFile->plagi_original_filename;
                       $uploaded_plagi_file['getfile']=$plagiFile->plagi_filename;
                       $uploaded_plagi_file['file_id']=$plagiFile->id;
                       $uploaded_plagi_file_array[]=$uploaded_plagi_file;
                   }   
               }
               else
               {
             ...
             else
             {
              ...
              if($details->solution_available==1)
              {
                  ...
                  if($availPlagiSolutionFile)
                  {
                       ...
                       {
                           $plagi_solution_files_data['filename']=$plagiSolnFile->plagi_original_filename;
                           $plagi_solution_files_data['getfile']=$plagiSolnFile->plagi_filename;
              ...
              $array_assignment_details['uploaded_plagi_file']  =  $uploaded_plagi_file_array;
