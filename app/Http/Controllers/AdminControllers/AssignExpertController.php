        ...
        { 
           ...
           { 
              ...
              return Redirect::to($redirect_url);      
               ...
               elseif($check_assignment_only->status!='accept' && $check_assignment_only->status='waiting')
              {
              Session::flash('assign-expert',"This request has been send to another expert");
              DB::table('acceptance_notification_logs')->update(['assignment_id'=>$assignment_id,'expert_id'=>$expert_id,'status'=>'waiting','timestamp'=>time()]);
              return Redirect::to($redirect_url);          
