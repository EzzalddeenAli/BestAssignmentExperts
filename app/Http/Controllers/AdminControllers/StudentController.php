...
class StudentController extends Controller 
{
 ...
 public function showAssignment()
    {
        return View('admin.dashboard.ShowAssignmentByDate');
    }   
  ...
  $address     = User::find($id)->address;
      ...
      return view('admin.dashboard.student-profile')->with(['id'=>$id,'name'=>$student_name,'email'=>$student_email,'mob'=>$mobile_no,
              ...
              'wallet_amount'=>$wallet_amount,'password'=>$password,'last_login'=>$last_login,'status'=>$status,'address'=>$address,'students_profile_data'=>$profile_data ]);  
     ...
     // dd($defaultImgPath);        
      ...
      // dd( $fullpath);
       ...
       DB::table('users')->where('id',$id)->update(['name'=>$name,'email'=>$email,'password'=>$pass,
                                  'contact_no'=>$mobile_no,'profile_image'=>$profileImgRealPath,
                                  'about_me'=>$about_me]);
      ...
      //Session::flash('Profile Updated successfully');//Need to alss check for /id so that directy any one could not able to delete the user
       return redirect('/student-profile/'.$id)->with('status','  Profile Image and Data sucessfully updated you can see updated image in student dashboard');  
}
else{

  DB::table('users')->where('id',$id)->update(['name'=>$name,'email'=>$email,'password'=>$pass,
                                  'contact_no'=>$mobile_no,'profile_image'=>$defaultImgPath,
                                  'about_me'=>$about_me]);
       
       
      //Session::flash('Profile Updated successfully');//Need to alss check for /id so that directy any one could not able to delete the user
       return redirect('/student-profile/'.$id)->with('status',' your Profile  sucessfully updated');  
}
   ...
   public function addAmountInUserWallet()
   {
         ...
         if(!ctype_digit($addAmount))
         {
          ...
          $getUserData = User::find($uid)?User::find($uid)->name:'';
            ...
            if($type==''||$type==null||empty($type))
           {
                  ...
                  {
                      return $response->responseData('true', 'You are going to add an amout of $'.$addAmount.'to the '.$getUserData.' wallet.', '');
                  } 
                  else
                  {
    ...
    public function ShowTotalAssignment($id){
        ...
        if($orders)
        {
            ...
            {   
                $payment_logs_array=DB::table('payment_logs')->where('order_id',$order->order_id)->get();
...
//dd($totalAssignments);
     ...
     public function ShowAssignmentByDate(Request $request)
    {
        $response = new SendResponseController(); 
         $firstdate =    strtotime($request->firstdate);
      $beginOfDay = strtotime("midnight", $firstdate);
...
//}
   ...
   //    $assignmentData=Assignment::where('uid',$id)->get();
   // dd($assignmentData);
...
//dd($u);
         // dd($assingnment_data['subtotal']);
          $put_in_array[]=$assingnment_data;
                 ...
                 return View('admin.dashboard.show-Assignment')->with(['assignment_data'=>$Assignment,'idFormation'=>$this->idForFormation]);
}
//else{
//  return redirect('/test1')->with('status','no record found for this date');
//}
}       
