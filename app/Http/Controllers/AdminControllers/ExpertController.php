...
$profile_data=DB::table('users')
      ...
      return view('admin.dashboard.expert-profile')->with(['id'=>$id,'name'=>$student_name,'email'=>$student_email,'mob'=>$mobile_no,
              ...
              'wallet_amount'=>$wallet_amount,'referrals'=>$referral_count,'password'=>$password,'last_login'=>$last_login,'status'=>$status,'students_profile_data'=>$profile_data]);  
...
//dd($r);
      ...
      Session::flash('status','Profile Updated successfully');//Need to alss check for /id so that directy any one could not able to delete the user
    return redirect('expert-profile/'.$id);  
 }
else{
   DB::table('users')->where('id',$id)->update(['name'=>$name,'email'=>$email,'password'=>$pass,
