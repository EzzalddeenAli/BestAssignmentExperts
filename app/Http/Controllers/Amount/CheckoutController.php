...
class CheckoutController extends Controller 
{
    ...
    { 
        ...
        $currency =  $checkAssigmentId->total_amount;
        $currency_value   =  explode(" ",$currency);
        $updated_currency =    $currency_value[1];
        if($checkAssigmentId)
        {
         ...
         if($id!=null||!empty($id))
         {
           ...
           return View('dashboard.checkout')->with(['assignment_id'=>$id,'show_assignment_id'=>$show_assignment_id,'currency'=>$updated_currency]);
         } 
