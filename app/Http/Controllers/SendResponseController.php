...
//                }
 ...
 public static function getAssignmentAsOrderId($assignment_id,$uid)
 {
        ...
        $detailsOfPlagiFile  =  DB::table('plagi_files')->where('assignment_id',$detailsOfAssignment->assignment_id)->count();
        
       if($detailsOfPlagiFile != null)
       {  
        $deadlineDdate = date("dmY",$detailsOfAssignment->due_date);
        $count=$data[0]->count;
        return 'PLG-'.$deadlineDdate.'-'.str_pad($uid, 4, '0', STR_PAD_LEFT).'-'.$count.'-'.$assignment_id ; 
       }
       else if($detailsOfPlagiFile == null)
       {  
        $deadlineDdate = date("dmY",$detailsOfAssignment->due_date);
        $count=$data[0]->count;
