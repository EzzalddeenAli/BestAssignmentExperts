...
function add_button_menu() {
  ...
  $('nav.main-nav li').each(function(){
    if ( $(this).children("ul").length > 0 ) {
      $(this).append("<span class='button_open dropdown-toggle'></span>");
    };
  })
}
...
function mobile_nav_handlers_init (){
  ...
  jQuery("nav.main-nav.mobile_nav .dropdown-toggle").on( "click", function (e){
    var el = jQuery(this);
    var next_id = el.closest("li").attr("id");
    var current_nav_level = get_current_nav_level();
    var next_nav_level = get_next_nav_level( next_id );
...
if ($(".contact-form").length) {
  ...
  var submit_handler =  function (form, wrapper){
    ...
    //send data to server
    $.post("/query-email-sent-onpage", data, function(s_response) {
        console.log(s_response);
     // s_response = $.parseJSON(s_response);
      if(s_response.success == 'success'){
        $wrapper.addClass("message message-success").append("<div class='info-boxes confirmation-message' id='feedback-form-success'><strong>Success!</strong><br><p>Thank you for getting in touch! We appreciate you contacting us. We will try to respond as soon as possible within a 24 hours. Have a great day ahead!</p></div>");
